...
namespace TicTacToe.Server {
    class Program {
        ...
        private static readonly ITcpListenerProvider _tcpListenerProvicer;

        static Program () {
            var kernel = new StandardKernel (new Bindings ());
            _tcpServerManager = kernel.Get<ITcpServerManager>();
            _tcpListenerProvicer = kernel.Get<ITcpListenerProvider>();
        }
        static void Main (string[] args) {          
            var listeningIP = IPAddress.Parse ("127.0.0.1");
            var server = _tcpListenerProvicer.CreateListener(listeningIP, _listeningPort);
